...
// FORM HANDLING
// ═══════════════════════════════════════════════════════════════════
function // Khởi tạo Supabase (Sử dụng đúng Key của anh)
...
async function handleSubmit() {
  ...
  submitBtn.innerText = 'Đang xử lý dữ liệu...';
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('leads')
      .insert([
        { full_name: name, phone: phone, email: email, status: 'new' }
      ]);

    if (error) throw error;

    // 2. NẾU LƯU THÀNH CÔNG -> HIỂN THỊ QR SEPAY
    ...
    alert('Hệ thống đang quá tải, anh/chị vui lòng bấm thử lại nhé!');
  } finally {
    // Khôi phục nút bấm
